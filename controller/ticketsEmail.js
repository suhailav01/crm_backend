const repo = require("../repositories/ticketsEmail");
const sendEmail = require("../utils/sendEmail");

// get emails by ticket id
const getEmailsByTicketId = async (req, res) => {
  try {
    const { ticketId } = req.params;

    const data = await repo.getEmailsByTicketId(ticketId);
...
const createEmail = async (req, res) => {
  try {
    ...
    if (!ticket_id || !recipients || !subject || !body) {
      return res.status(400).json({
        success: false,
        message: "ticket_id, recipients, subject and body are required",
      });
    }
