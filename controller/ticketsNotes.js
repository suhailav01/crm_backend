const repo = require("../repositories/ticketsNotes");
...
const getNotesByTicketId = async (req, res) => {
  try {
    ...
    const data = await repo.getNotesByTicketId(ticketId);
...
const createNote = async (req, res) => {
  try {
    const { ticket_id, note_text, created_by } = req.body;

    if (!ticket_id || !note_text) {
      return res.status(400).json({
        success: false,
        message: "ticket_id and note_text are required",
      });
    }
