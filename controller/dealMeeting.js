...
const repo = require("../repositories/dealMeetings");

// get meetings by deal id
const getMeetingsByDealId = async (req, res) => {
  try {
    const { dealId } = req.params;

    const data = await repo.getMeetingsByDealId(dealId);
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "deal_id, title, start_date, start_time, end_time and attendees are required",
      });
    }
