const repo = require("../repositories/company_tasks");

// get tasks by company id
const getTasksByCompanyId = async (req, res) => {
  try {
    const { companyId } = req.params;

    const data = await repo.getTasksByCompanyId(companyId);
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "company_id, task_name, due_date, due_time, task_type and priority are required",
      });
    }
