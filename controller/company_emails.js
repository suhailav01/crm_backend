const repo = require("../repositories/company_emails");
const sendEmail = require("../utils/sendEmail");

// get emails by company id
const getEmailsByCompanyId = async (req, res) => {
  try {
    const { companyId } = req.params;

    const data = await repo.getEmailsByCompanyId(companyId);
...
const createEmail = async (req, res) => {
  try {
    ...
    if (!company_id || !recipients || !subject || !body) {
      return res.status(400).json({
        success: false,
        message: "company_id, recipients, subject and body are required",
      });
    }
