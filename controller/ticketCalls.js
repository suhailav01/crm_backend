...
// get calls by ticket id
const getCallsByTicketId = async (req, res) => {
  try {
    const { ticketId } = req.params;

    const data = await repo.getCallsByTicketId(ticketId);
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "ticket_id, connected_to, call_outcome, call_date and call_time are required",
      });
    }
...
const makeCall = async (req, res) => {
  try {
    const { ticket_id, customer_phone, connected_to, created_by } = req.body;
