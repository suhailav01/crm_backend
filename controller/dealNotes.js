const repo = require("../repositories/dealNotes");

// get notes by deal id
const getNotesByDealId = async (req, res) => {
  try {
    const { dealId } = req.params;

    const data = await repo.getNotesByDealId(dealId);
...
const createNote = async (req, res) => {
  try {
    const { deal_id, note_text, created_by } = req.body;

    if (!deal_id || !note_text) {
      return res.status(400).json({
        success: false,
        message: "deal_id and note_text are required",
      });
    }
